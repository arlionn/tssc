...
{p 4 8}{cmd:average_effect(}{it:string}{cmd:)}: if that option is specified, the command will compute an average of the instantaneous and dynamic effects requested. If {cmd:average_effect(}{it:simple}{cmd:)} is specified, the command will compute the simple average of the effects and its standard error. If {cmd:average_effect(}{it:prop_number_switchers}{cmd:)} is specified, the command will compute an average where each effect receives a weight proportional to the number of switchers the effect applies to. If {cmd:average_effect(}{it:minvar}{cmd:)} is specified, the command will compute an approximation of the average of the effects with the lowest variance. The weights w of the optimal average minimize w'Vw, where V is the variance-covariance matrix of the effects, subject to the weights being positive and summing to 1. This problem does not have a closed-form solution, and Stata does not have a quadratic programming solver to approximate it. When the covariances between all the effects are equal, the problem has a closed-form solution, so when this option is specified the command returns the optimal estimator if all the covariances were equal to their average. In practice, the covariances between the effects are unlikely to all be equal, but if they are not too different then the average effect computed by the command should be close to the optimal one. When {cmd:average_effect} is specified, the {cmd:covariances} option also has to be specified, and the number of dynamic effects requested should be greater than or equal to 1.{p_end}
