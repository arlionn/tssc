...
// Weights for average of effects with lowest variance, assuming equal covariances

if "`average_effect'"=="minvar"{

*Computing average covariance

scalar cov=0
forvalue i=0/`=`dynamic''{
if `i'<`dynamic'{
forvalue j=`=`i'+1'/`=`dynamic''{
scalar cov=cov+cov_effects_`i'`j'_int
}
}
}
scalar cov=cov/((`dynamic'+1)*`dynamic')

*Checking if cov lower than all the variances

scalar check_cov=1
forvalue i=0/`=`dynamic''{
scalar check_cov_temp=(cov<=se_effect_`i'_2^2)
scalar check_cov=min(check_cov,check_cov_temp)
}
if check_cov==0{
noisily di as text "Given the variance-covariance matrix of the estimated effects, the approximation"
noisily di as text "of the average of the effects with lowest variance cannot be computed. You can"
noisily di as text "compute the actual average of the effects with lowest variance using the variance-covariance" 
noisily di as text "matrix of the estimated effects and a software equipped with a quadratic programming solver."
}

if check_cov==1{
*Computing denominator of weights

scalar denom=0
forvalue i=0/`=`dynamic''{
scalar denom=denom+max(0,1/(se_effect_`i'_2^2-cov))
}
...
forvalue i=0/`=`dynamic''{
matrix Weight[`i'+1,1]=max(0,1/(se_effect_`i'_2^2-cov))/denom
}
