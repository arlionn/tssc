...
program define tsscinstall
	* tssc install <package> [, <net_install_options>]
	...
	if _rc != 0 {
		...
		local rc _rc
		if _rc != 0 {
			di as err `"Failed Again! If you are sure that this package is on TSSC, you can try to download `pkgname' from {browse "https://czxa.github.io/tssc/ssczip/`pkgname'.zip": `pkgname'.zip} manually, unzip and install it by running:"' _n in yellow `"net install `pkgname'.pkg, from("where the folder `pkgname' is")"' _n as err `"If not, type {stata search `pkgname'}"'
			exit `rc'
		}
		if _rc == 0 {
			di in green "Succeeded!"
